...
export async function addSampleData() {
  try {
    ...
    const database = db;
    if (!database) {
      throw new Error("Firebase is not configured. Cannot add sample data.");
    }

    // Add sponsorship applications
    for (const sponsorship of sampleSponsorships) {
      const docRef = await addDoc(
        collection(database, "sponsorshipApplications"),
        sponsorship
      );
    ...
    for (const event of sampleEvents) {
      const docRef = await addDoc(collection(database, "events"), event);
