...
const UserManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const database = db;
    if (!database) {
      console.warn("[AdminUsers] Firebase is not configured.");
      setLoading(false);
      return;
    }
  ...
  const handleToggleApproval = async (
    ...
    try {
      ...
      const targetSnap = await getDoc(doc(database, "users", userId));
  ...
  const handleSaveUser = async () => {
    try {
      ...
      if (editingUserId) {
        ...
        await updateDoc(doc(database, "users", editingUserId), {
  ...
  const handleUpdateCalls = async () => {
    ...
    try {
      ...
      await updateDoc(doc(database, "users", callsTargetUserId), {
                            ...
                            (currentRole === "admin" ||
                              ...
                              <>
                                ...
                                {(currentRole === "super_admin" ||
                                  ...
                                  updateDoc(doc(database, "users", userData.id), {
                                    userType: newRole,
                                  }).then(() =>
                                    ...
                                    onClick={async () => {
                                      try {
                                        ...
                                        const targetSnap = await getDoc(
                                          doc(database, "users", userData.id)
                                        );
                                        if (targetSnap.exists()) {
                                          const td =
