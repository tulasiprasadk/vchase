...
export const useVerification = () => {
  ...
  const fetchVerificationRequests = async () => {
    try {
      ...
      const database = db;
      if (!database) {
        console.warn("[useVerification] Firebase is not configured.");
        setLoading(false);
        return undefined;
      }
      const q = query(
        collection(database, "verificationRequests"),
  ...
  const approveVerification = async (
    ...
    try {
      ...
      const requestRef = doc(database, "verificationRequests", requestId);
      ...
      // Update user profile
      await updateDoc(doc(database, "users", request.userId), {
        isVerified: true,
