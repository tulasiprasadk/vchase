  ...
  const getUserRole = useCallback(
    async (userId: string): Promise<"organizer" | "sponsor"> => {
      try {
        const database = db;
        if (!database) {
          console.warn("[useMessaging] Firebase is not configured.");
          return "sponsor";
        }
  ...
  const findExistingChat = useCallback(
    async (
      ...
      try {
        ...
        if (!database) {
          console.warn("[useMessaging] Realtime Database is not configured.");
          return null;
        }
  ...
  const createChat = useCallback(
    async (
      ...
      try {
        ...
        await update(ref(database, `chats/${chatId}/metadata`), cleanedChatMetadata);
        ...
        await update(
          ref(database, `chats/${chatId}/messages/${messageData.id}`),
          messageData
        );
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        const database = realtimeDb;
        if (!database) {
          toast.error("Messaging service is not available.");
          return;
        }
        ...
        await update(
          ref(database, `chats/${chatId}/metadata`),
          metadataUpdate
        );

        // Update unread counts for other participants
        const chatSnapshot = await get(
          ref(database, `chats/${chatId}/metadata/participants`)
        );
  ...
  const markAsRead = useCallback(
    async (chatId: string): Promise<void> => {
      ...
      try {
        ...
        await update(ref(database, `chats/${chatId}/metadata`), updates);
      } catch (error) {
